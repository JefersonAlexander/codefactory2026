...
export default function ReportsPage() {
  ...
  const comparisonData = [
    {
      name: "Ingresos",
      planificado: currentBudget.totalBudget,
      real: report.totalIncome,
    },
    {
      name: "Gastos",
      planificado: currentBudget.totalBudget * 0.85,
      real: report.totalExpenses,
    },
  ];

  const trendData = monthlyReports
    .slice()
    .reverse()
    .map((r) => ({
  ...
  return (
    <>
      <DashboardHeader
        ...
        subtitle={`Analiza tu situación financiera, ${user.nombre}`}
      />
        ...
        <Card className={`${status.bg} ${status.border} border`}>
          <CardContent className="py-6">
            <div className="flex items-center gap-4">
              <div
                className={`w-14 h-14 rounded-xl ${status.bg} flex items-center justify-center`}
              >
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <Badge
                    className={`${status.bg} ${status.color} border ${status.border}`}
                  >
                    {report.status === "saving" ? "+" : ""}
                    {formatCurrency(report.balance, user.currency)}
                  </Badge>
                </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-success">
                {formatCurrency(report.totalIncome, user.currency)}
              </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-destructive">
                {formatCurrency(report.totalExpenses, user.currency)}
              </div>

              <p className="text-xs text-muted-foreground mt-2">
                {Math.round((report.totalExpenses / report.totalIncome) * 100)}
                % de ingresos
              </p>
            </CardContent>
          </Card>
                      ...
                      value={
                        report.totalIncome > 0
                          ? Math.max(
                              0,
                              Math.round(
                                (report.balance / report.totalIncome) * 100
                              )
                            )
                          : 0
                      }
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-4 rounded-lg bg-muted/50">
                      ...
                      <p className="text-lg font-semibold">
                        {formatCurrency(report.totalExpenses / 30, user.currency)}
                      </p>
                    </div>
                    ...
                    <div className="p-4 rounded-lg bg-muted/50">
                      ...
                      <p className="text-lg font-semibold">
                        {formatCurrency(report.totalIncome / 30, user.currency)}
                      </p>
                    </div>
                  </div>
          ...
          <TabsContent value="trends" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Evolución de ingresos, gastos y balance
                </CardDescription>
              </CardHeader>
                  ...
                  >
                    <CardContent className="pt-6">
                      <div className="flex items-center justify-between mb-4">
                        ...
                        <div
                          className={`w-10 h-10 rounded-full ${rStatus.bg} flex items-center justify-center`}
                        >
                          <RStatusIcon
                            className={`h-5 w-5 ${rStatus.color}`}
                          />
                        </div>
                      </div>
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="text-success font-medium">
                            {formatCurrency(r.totalIncome, user.currency)}
                          </span>
                        </div>
                        ...
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="text-destructive font-medium">
                            {formatCurrency(r.totalExpenses, user.currency)}
                          </span>
                        </div>
                        ...
                        <div className="flex justify-between text-sm pt-2 border-t">
                          ...
                          >
                            {formatCurrency(r.balance, user.currency)}
                          </span>
                        </div>
                      </div>
                    </CardContent>
